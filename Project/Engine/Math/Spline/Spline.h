...
public:
    ...
    /// @param color 線の色
    /// @param alpha 線のアルファ値
    /// @return LineRenderer用の線分配列
    std::vector<LineRenderer::Line> GenerateLines(int segments = 50, const Vector3& color = {1.0f, 1.0f, 1.0f}, float alpha = 1.0f) const;

    /// @brief コントロールポイント表示用の線分を生成
    /// @param color 線の色
    /// @param alpha 線のアルファ値
    /// @return LineRenderer用の線分配列
    std::vector<LineRenderer::Line> GenerateControlPointLines(const Vector3& color = {1.0f, 0.0f, 0.0f}, float alpha = 1.0f) const;
