...
TextureManager::LoadedTexture TextureManager::Load(const std::string& filePath)
{
	...
	std::wstring filePathW = Logger::GetInstance().ConvertString(filePath);
	...
	// メタデータをキャッシュに保存
	metadataCache_[filePath] = texMetadata;
	...
	if (FAILED(hr)) {
		throw std::runtime_error("Failed to create texture resource: " + filePath);
	}
...
DirectX::TexMetadata TextureManager::GetMetadata(const std::string& filePath)
{
	...
	if (FAILED(hr)) {
		throw std::runtime_error("Failed to load texture for metadata: " + filePath);
	}
