...
void RotationModule::ApplyInitialRotation(Particle& particle)
{
    ...
    if (rotationData_.use2DRotation) {
        // 2D回転モード（Z軸のみ）
        float initialRotationZ = rotationData_.startRotation.z;
        
        // ランダム性を適用
        if (rotationData_.rotationSpeedRandomness.z > 0.0f) {
            initialRotationZ = ApplyRandomness(initialRotationZ, rotationData_.rotationSpeedRandomness.z);
        }
        
        particle.transform.rotate = {0.0f, 0.0f, initialRotationZ};
        
        // 2D回転速度を設定（Particle構造体の回転速度を使用）
        ...
        // 3D回転モード
        Vector3 initialRotation = rotationData_.startRotation;
        
        // 各軸にランダム性を適用
        initialRotation.x = ApplyRandomness(initialRotation.x, rotationData_.rotationSpeedRandomness.x);
        initialRotation.y = ApplyRandomness(initialRotation.y, rotationData_.rotationSpeedRandomness.y);
        initialRotation.z = ApplyRandomness(initialRotation.z, rotationData_.rotationSpeedRandomness.z);
        
        particle.transform.rotate = initialRotation;
...
bool RotationModule::ShowImGui() {
    ...
    if (rotationData_.use2DRotation) {
        ...
        // 3D回転設定
        changed |= ImGui::DragFloat3("開始回転", &rotationData_.startRotation.x, 0.1f);
