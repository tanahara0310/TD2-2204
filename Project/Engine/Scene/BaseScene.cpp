...
void BaseScene::Draw()
{
	...
	// 追加の描画可能オブジェクトを描画キューに追加
	for (IDrawable* drawable : additionalDrawables_) {
		if (drawable) {
...
void BaseScene::AddDrawable(IDrawable* drawable)
{
	if (drawable) {
		additionalDrawables_.push_back(drawable);
	}
}
