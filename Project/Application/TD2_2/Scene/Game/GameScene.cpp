...
void GameScene::Initialize(EngineSystem* engine) {
   ...
   // ボスの生成と初期化
   {
	  ...
	  // ビヘイビアツリー再構築コールバックを設定（ダメージ時にツリーをリセット）
...
std::unique_ptr<BehaviorTree> GameScene::CreateBossBehaviorTree() {
   ...
   // ボスから中央への距離の評価基準となる最大範囲
   const float max_range = 30.0f;

   // Bossから中央への距離を取得する関数
   auto get_center_distance = [this, center]() {
	  return MathCore::Vector::Length(boss_->GetWorldPosition() - center);
   };

   // 【HP=3/HP=1用】中央に近いほど高評価 (0.0fで1.0f, max_rangeで0.0f)
   auto create_near_center_evaluator = [get_center_distance, max_range]() {
	  return std::make_unique<LambdaEvaluator>([get_center_distance, max_range]() {
		 float dist = get_center_distance();
		 return std::clamp(1.0f - (dist / max_range), 0.0f, 1.0f);
	  });
   };

   // 【HP=3/HP=1用】中央から遠いほど高評価 (0.0fで0.0f, max_rangeで1.0f)
   auto create_far_center_evaluator = [get_center_distance, max_range]() {
	  return std::make_unique<LambdaEvaluator>([get_center_distance, max_range]() {
		 float dist = get_center_distance();
		 return std::clamp(dist / max_range, 0.0f, 1.0f);
	  });
   };

   // 【HP=2/HP=1用】Playerがスタンしている時高評価 (1.0f)
   auto create_stun_bias_evaluator = [this]() {
	  return std::make_unique<LambdaEvaluator>([this]() {
		 if (player_->IsStunned()) {
			return 1.0f;
		 }
		 return 0.1f; // スタンしていない時は低め
	  });
   };

   // 【HP=1用】Playerが非スタン時に高評価 (1.0f)
   auto create_non_stun_bias_evaluator = [this]() {
	  return std::make_unique<LambdaEvaluator>([this]() {
		 if (!player_->IsStunned()) {
			return 1.0f;
		 }
   ...
   // 【NEW】プレイヤーとの距離が近い時に高評価（突進を選びやすく）
   auto create_close_to_player_evaluator = [this]() {
	  return std::make_unique<LambdaEvaluator>([this]() {
		 float dist = MathCore::Vector::Length(boss_->GetWorldPosition() - player_->GetWorldPosition());
		 // 15.0f以下で1.0f、30.0f以上で0.0f
		 return std::clamp(1.0f - (dist - 15.0f) / 15.0f, 0.0f, 1.0f);
	  });
   };

   // 【HP=1用】距離が近い時に高評価（突進攻撃用）
   auto create_close_range_evaluator = [this]() {
	  return std::make_unique<LambdaEvaluator>([this]() {
		 float dist = MathCore::Vector::Length(boss_->GetWorldPosition() - player_->GetWorldPosition());
		 // 20.0f以下で1.0f、40.0f以上で0.0f
		 return std::clamp(1.0f - (dist - 20.0f) / 20.0f, 0.0f, 1.0f);
	  });
   };

   // 【HP=1用】適度な距離を評価（射撃攻撃用）
   auto create_medium_range_evaluator = [this]() {
	  return std::make_unique<LambdaEvaluator>([this]() {
		 float dist = MathCore::Vector::Length(boss_->GetWorldPosition() - player_->GetWorldPosition());
		 // 15.0f～25.0fが最適範囲
		 if (dist < 15.0f) {
			return std::clamp(dist / 15.0f, 0.0f, 1.0f);
		 } else if (dist > 25.0f) {
			return std::clamp(1.0f - (dist - 25.0f) / 15.0f, 0.0f, 1.0f);
		 }
		 return 1.0f;
	  });
   };
   ...
   // ボスHPの現在値を取得する関数
   auto get_boss_hp = [this]() { return boss_->GetHP(); };

   // ----------------------------------------------------------------------
   // ビヘイビアツリー構築（全フェーズをSelectorで管理）
   // ----------------------------------------------------------------------
   return BehaviorTreeFactory::Create(
	  [this, get_boss_hp, create_near_center_evaluator, create_far_center_evaluator, 
	   create_stun_bias_evaluator, create_non_stun_bias_evaluator,
	   create_close_range_evaluator, create_medium_range_evaluator, create_close_to_player_evaluator](BehaviorTreeBuilder& builder) {
		 builder.Selector() // ルート: Selector（最初に成功したフェーズを実行）

			// ==========================================================
			// 【HP=5 フェーズ】: FleeFromPlayerAction のみ（逃げるだけ）
			// ==========================================================
			.Sequence()
			.Condition([get_boss_hp]() { return get_boss_hp() == 5; })
			.Action<FleeFromPlayerAction>(boss_, player_)
			.End()

			// ==========================================================
			// 【HP=4 フェーズ】: 基本的な攻撃パターン導入
			// Flee -> Charge -> WeightedSelector(Charge 0.6, MoveToCenter 0.4)
			// ==========================================================
			.Sequence()
			.Condition([get_boss_hp]() { return get_boss_hp() == 4; })
			.Action<FleeFromPlayerAction>(boss_, player_)
			.Action<ChargeToPlayerAction>(boss_, player_)
			.WeightedSelector()
			.WeightedAction<ChargeToPlayerAction>(0.6f, boss_, player_)
			.WeightedAction<MoveToCenterAction>(0.4f, boss_)
			.End()
			.End()

			// ==========================================================
			// 【HP=3 フェーズ】: 突進とショットのみを使用（増量）
			// シーケンスで行動パターンを明確化
			// ==========================================================
			.Sequence()
			.Condition([get_boss_hp]() { return get_boss_hp() == 3; })
			.Action<FleeFromPlayerAction>(boss_, player_)
			.WeightedSelector()
			
			// パターン1: 突進のみ（近距離で選ばれやすい）
			.WeightedNode(
			   [this]() {
				  auto sequence = std::make_unique<SequenceNode>();
				  sequence->AddChild(std::make_unique<ChargeToPlayerAction>(boss_, player_));
				  return sequence;
			   }(),
			   [this, create_close_to_player_evaluator]() {
				  auto composite = std::make_unique<CompositeEvaluator>(CompositeEvaluator::CombineMode::Product);
				  composite->AddEvaluator(create_close_to_player_evaluator(), aiParams_.hp3.chargeOnly);
				  return composite;
			   }()
			)
			
			// パターン2: 突進→逃げ（ヒットアンドアウェイ）
			.WeightedNode(
			   [this]() {
				  auto sequence = std::make_unique<SequenceNode>();
				  sequence->AddChild(std::make_unique<ChargeToPlayerAction>(boss_, player_));
				  sequence->AddChild(std::make_unique<FleeFromPlayerAction>(boss_, player_));
				  return sequence;
			   }(),
			   aiParams_.hp3.chargeAndFlee
			)
			...
			.WeightedNode(
			   [this]() {
				  auto sequence = std::make_unique<SequenceNode>();
				  sequence->AddChild(std::make_unique<ShootEightWayAction>(boss_, [this](const Vector3& pos, const Vector3& direction, float speed) { 
					 CreateBullet(pos, direction, BulletType::ElasticSphere, speed); 
				  }));
				  return sequence;
			   }(),
			   [this, create_near_center_evaluator]() {
				  auto composite = std::make_unique<CompositeEvaluator>(CompositeEvaluator::CombineMode::Product);
				  composite->AddEvaluator(create_near_center_evaluator(), aiParams_.hp3.shootOnly);
				  return composite;
			   }()
			)
			...
			// パターン6: 中央移動→突進（位置取り重視）
			.WeightedNode(
			   [this]() {
				  auto sequence = std::make_unique<SequenceNode>();
				  sequence->AddChild(std::make_unique<MoveToCenterAction>(boss_));
				  sequence->AddChild(std::make_unique<ChargeToPlayerAction>(boss_, player_));
				  return sequence;
			   }(),
			   [this, create_far_center_evaluator]() {
				  auto composite = std::make_unique<CompositeEvaluator>(CompositeEvaluator::CombineMode::Product);
				  composite->AddEvaluator(create_far_center_evaluator(), aiParams_.hp3.moveCenterAndCharge);
				  return composite;
			   }()
			)
			...
			// ==========================================================
			// 【HP=2 フェーズ】: 突進とスパークのみを使用
			// スパーク前に必ず突進2回（前隙を作る）
			// ==========================================================
			.Sequence()
			.Condition([get_boss_hp]() { return get_boss_hp() == 2; })
			.WeightedSelector()
			
			// パターン1: 逃げ→突進×2→スパーク→突進（基本コンボ）
			.WeightedNode(
			   [this]() {
				  auto sequence = std::make_unique<SequenceNode>();
				  sequence->AddChild(std::make_unique<FleeFromPlayerAction>(boss_, player_));
				  sequence->AddChild(std::make_unique<ChargeToPlayerAction>(boss_, player_));
				  sequence->AddChild(std::make_unique<ChargeToPlayerAction>(boss_, player_)); // 前隙
				  sequence->AddChild(std::make_unique<SparkNode>(boss_, sparkCollider_));
				  sequence->AddChild(std::make_unique<ChargeToPlayerAction>(boss_, player_)); // 押し込み
				  return sequence;
			   }(),
			   0.5f // 基本戦術
			)
			
			// パターン2: 突進×2→スパーク→突進（接近戦、近距離で優先）
			.WeightedNode(
			   [this]() {
				  auto sequence = std::make_unique<SequenceNode>();
				  sequence->AddChild(std::make_unique<ChargeToPlayerAction>(boss_, player_));
			   ...
			   [create_close_to_player_evaluator]() {
				  auto composite = std::make_unique<CompositeEvaluator>(CompositeEvaluator::CombineMode::Product);
				  composite->AddEvaluator(create_close_to_player_evaluator(), 1.0f); // 近距離で選ばれやすい
				  return composite;
			   }()
			)
			
			// パターン3: スタン中の追撃 - 連続突進×3
			.WeightedNode(
			   [this]() {
				  auto sequence = std::make_unique<SequenceNode>();
				  sequence->AddChild(std::make_unique<ChargeToPlayerAction>(boss_, player_));
			   ...
			   create_stun_bias_evaluator() // スタン時バイアス
			)
			...
			// ==========================================================
			// 【HP=1 フェーズ】: 最強AI - 全アクション駆使
			// スパークは必ず突進×2の後、近距離で突進優先
			// ==========================================================
			.Sequence()
			.Condition([get_boss_hp]() { return get_boss_hp() <= 1; })
			.WeightedSelector()
			
			// 戦略1: 中央への移動（壁から遠い時は中央確保を優先）
			.WeightedNode(
			   [this]() {
				  auto sequence = std::make_unique<SequenceNode>();
				  sequence->AddChild(std::make_unique<MoveToCenterAction>(boss_));
			   ...
			   [create_far_center_evaluator]() {
				  auto composite = std::make_unique<CompositeEvaluator>(CompositeEvaluator::CombineMode::Product);
				  composite->AddEvaluator(create_far_center_evaluator(), 1.5f);
				  return composite;
			   }()
			)
			
			// 戦略2: 突進×2→スパーク→突進（近距離＆非スタン時優先）
			.WeightedNode(
			   [this]() {
				  auto sequence = std::make_unique<SequenceNode>();
				  sequence->AddChild(std::make_unique<ChargeToPlayerAction>(boss_, player_));
			   ...
			   [create_close_to_player_evaluator, create_non_stun_bias_evaluator]() {
				  auto composite = std::make_unique<CompositeEvaluator>(CompositeEvaluator::CombineMode::Product);
				  composite->AddEvaluator(create_close_to_player_evaluator(), 1.0f);
				  composite->AddEvaluator(create_non_stun_bias_evaluator(), 0.8f);
				  return composite;
			   }()
			)
			...
			.WeightedNode(
			   ...
			   [create_medium_range_evaluator, create_near_center_evaluator]() {
				  auto composite = std::make_unique<CompositeEvaluator>(CompositeEvaluator::CombineMode::Product);
				  composite->AddEvaluator(create_medium_range_evaluator(), 0.7f);
				  composite->AddEvaluator(create_near_center_evaluator(), 0.5f);
				  return composite;
			   }()
			)
			...
			.WeightedNode(
			   ...
			   [create_close_range_evaluator, create_near_center_evaluator]() {
				  auto composite = std::make_unique<CompositeEvaluator>(CompositeEvaluator::CombineMode::Product);
				  composite->AddEvaluator(create_close_range_evaluator(), 1.2f); // 近距離で強化
				  composite->AddEvaluator(create_near_center_evaluator(), 0.8f);
				  return composite;
			   }()
			)
			
			// 戦略5: スタン追撃コンボ（スタン時最優先）
			.WeightedNode(
			   [this]() {
				  auto sequence = std::make_unique<SequenceNode>();
				  sequence->AddChild(std::make_unique<ChargeToPlayerAction>(boss_, player_));
				  sequence->AddChild(std::make_unique<ChargeToPlayerAction>(boss_, player_));
				  sequence->AddChild(std::make_unique<ChargeToPlayerAction>(boss_, player_));
				  sequence->AddChild(std::make_unique<ChargeToPlayerAction>(boss_, player_)); // 4連
				  return sequence;
			   }(),
			   [create_stun_bias_evaluator]() {
				  auto composite = std::make_unique<CompositeEvaluator>(CompositeEvaluator::CombineMode::Product);
				  composite->AddEvaluator(create_stun_bias_evaluator(), 2.0f);
				  return composite;
			   }()
			)
			...
			.WeightedNode(
			   [this]() {
				  auto sequence = std::make_unique<SequenceNode>();
				  sequence->AddChild(std::make_unique<ChargeAwayFromPlayerAction>(boss_, player_));
				  sequence->AddChild(std::make_unique<ShootEightWayAction>(boss_, [this](const Vector3& pos, const Vector3& direction, float speed) { 
					 CreateBullet(pos, direction, BulletType::ElasticSphere, speed); 
				  }));
				  return sequence;
			...
			// 戦略7: 逃げ→突進×2→スパーク→突進（遠距離スパークコンボ）
			.WeightedNode(
			   [this]() {
				  auto sequence = std::make_unique<SequenceNode>();
				  sequence->AddChild(std::make_unique<FleeFromPlayerAction>(boss_, player_));
			   ...
			   [create_far_center_evaluator]() {
				  auto composite = std::make_unique<CompositeEvaluator>(CompositeEvaluator::CombineMode::Product);
				  composite->AddEvaluator(create_far_center_evaluator(), 0.6f); // 遠い時
				  return composite;
			   }()
			)
			...
			.End() // WeightedSelector
			.End() // Sequence

			// ==========================================================
			// 【フォールバック】: 全てのHP条件に一致しない場合の安全策
			// ==========================================================
			.Sequence()
			.Action<FleeFromPlayerAction>(boss_, player_)
			.End()
...
void GameScene::InitializeGameOver() {
   ...
   playerHitPointUI_->Clear();

   bossHitPointUI_->Clear();
