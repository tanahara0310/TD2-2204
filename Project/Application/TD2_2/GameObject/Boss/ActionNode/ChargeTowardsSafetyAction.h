...
/// @brief 壁から逃げる方向に突進するアクション（自滅防止）
class ChargeTowardsSafetyAction : public BossActionNode {
public:
   /// @brief コンストラクタ
   /// @param boss ボスへの参照
   ...
   ChargeTowardsSafetyAction(Boss* boss, Player* player, 
                             float chargeSpeed = 5000.0f, 
                             float chargeDuration = 0.3f);

   ~ChargeTowardsSafetyAction() override = default;
...
private:
   ...
   /// @brief 安全な方向（中央方向かつプレイヤーから離れる）を計算
   Vector3 CalculateSafeDirection() const;
