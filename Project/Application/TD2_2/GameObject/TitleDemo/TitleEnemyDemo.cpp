...
void TitleEnemyDemo::Update() {
	...
	if (isChasing_ && target_) {
		...
		// 距離が十分離れている場合のみ追跡移動
		if (distance > 0.1f) {
			transform_.translate.x += direction.x * chaseSpeed_ * deltaTime;
			transform_.translate.y += direction.y * chaseSpeed_ * deltaTime;
			transform_.translate.z += direction.z * chaseSpeed_ * deltaTime;
		}
		...
		// 通常移動モード（X軸方向）
		transform_.translate.x += chaseSpeed_ * moveDirection_ * deltaTime;
