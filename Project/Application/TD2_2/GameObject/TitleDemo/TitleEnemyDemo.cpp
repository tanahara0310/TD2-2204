...
void TitleEnemyDemo::UpdateChaseMode(float deltaTime) {
	...
	// 追跡移動
	if (distance > 0.1f) {
		transform_.translate.x += direction.x * chaseSpeed_ * deltaTime; // kInitialSpeed_ → chaseSpeed_
		// Y・Z座標は初期位置を維持
...
void TitleEnemyDemo::UpdateNormalMode(float deltaTime) {
	// 通常移動（X軸方向）
	transform_.translate.x += chaseSpeed_ * moveDirection_ * deltaTime; // kInitialSpeed_ → chaseSpeed_
