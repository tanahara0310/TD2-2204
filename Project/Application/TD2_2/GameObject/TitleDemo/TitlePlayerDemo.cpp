...
void TitlePlayerDemo::UpdateChaseMode(float deltaTime) {
	// ターゲット位置と方向の計算
...
void TitlePlayerDemo::UpdateRotation(float directionX, float deltaTime) {
	// 移動方向に応じて基本回転を決定
	float baseRotationZ;
	if (directionX > 0.0f) {
		// +X方向（右）に移動
...
void TitlePlayerDemo::SetBasicRotation(float direction) {
	if (direction > 0.0f) {
...
void TitlePlayerDemo::ResetRotationState() {
	rotationState_ = RotationState::Rotating;
	rotationTimer_ = 0.0f;
	waitTimer_ = 0.0f;
}
